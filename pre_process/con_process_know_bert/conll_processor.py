...
class InputFeature(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, boundary_ids, pos_ids, input_len, rel_ids,
                 knowledge_segment_ids=None, knowledge_input_ids=None, knowledge_input_mask=None):
        ...
        self.knowledge_input_mask = knowledge_input_mask
...
class ConllProcessor(object):
    ...
    def create_features(self, examples, cached_features_file, max_seq_len):
        ...
        for ex_id, example in enumerate(examples):
            ...
            if example.knowledge:
                ...
                knowledge_input_ids = []
                knowledge_input_mask = []
                for knowledge in example.knowledge:
                    know_tokens = self.tokenizer.tokenize(knowledge)
                    know_tokens = ['[CLS]'] + know_tokens + ['[SEP]']
                    know_segment_ids = [0] * len(know_tokens)
                    know_input_ids = self.tokenizer.convert_tokens_to_ids(know_tokens)
                    know_input_mask = [1] * len(know_tokens)

                    padding = [0] * (max_know_len - len(know_tokens))
                    know_segment_ids += padding
                    know_input_ids += padding
                    know_input_mask += padding
                    knowledge_segment_ids.append(know_segment_ids)
                    knowledge_input_ids.append(know_input_ids)
                    knowledge_input_mask.append(know_input_mask)

                padding = [[0] * max_know_len] * (max_seq_len - len(knowledge_input_ids))
                knowledge_segment_ids += padding
                knowledge_input_ids += padding
                knowledge_input_mask += padding
    ...
    def create_dataset(self, features, is_sorted=False):
        ...
        if features[0].knowledge_segment_ids:
            # all_know_ids = torch.tensor([f.know_ids for f in features], dtype=torch.long)
            all_knowledge_segment_ids = torch.tensor([f.knowledge_segment_ids for f in features], dtype=torch.long)
            all_knowledge_input_ids = torch.tensor([f.knowledge_input_ids for f in features], dtype=torch.long)
            all_knowledge_input_mask = torch.tensor([f.knowledge_input_mask for f in features], dtype=torch.long)
        else:
            all_knowledge_segment_ids = torch.tensor([0 for f in features], dtype=torch.long)
            all_knowledge_input_ids = torch.tensor([0 for f in features], dtype=torch.long)
            all_knowledge_input_mask = torch.tensor([0 for f in features], dtype=torch.long)

        dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_boundary_ids, all_pos_ids,
                                all_rel_ids, all_knowledge_segment_ids,all_knowledge_input_ids, all_knowledge_input_mask)
        return dataset
