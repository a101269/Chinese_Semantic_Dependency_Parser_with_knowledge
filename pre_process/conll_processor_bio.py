...
class InputExample(object):
    def __init__(self, guid, text_a, pos=None, knowledge=None, boundary=None, arcs=None,bio_tag=None):
...
class InputFeature(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, boundary_ids, pos_ids, input_len, rel_ids,
                 know_ids=None, know_adj_mar=None, bio_ids=None):
...
class ConllProcessor(object):
    ...
    def read_data(self, data_file):
        ...
        data = conll_file.get(['word', 'upos', 'deps', 'misc','feats'], as_sentences=True)  # 没有头节点原因 5:rAgt|12:rAgt
    ...
    def create_examples(self, lines, example_type, cached_examples_file):
        ...
        for i, line in enumerate(lines):
            ...
            bio_tag.append('<eos>')
            if i == len(lines) - 1:
                print(guid, pos, knowledge, boundary, arcs,bio_tag)
            text_a = ''.join(text_a)
            example = InputExample(guid=guid, text_a=text_a, pos=pos, knowledge=knowledge, boundary=boundary, arcs=arcs,bio_tag=bio_tag)
    ...
    def create_features(self, examples, cached_features_file, max_seq_len):
        ...
        for ex_id, example in enumerate(examples):
            ...
            assert isinstance(know_ids,list)
            bio_tag=example.bio_tag
            bio_ids=self.vocabs['bio'].map(bio_tag)
            padding = [0] * (max_seq_len - len(bio_ids))  # 0 的话会与CLS 位置搞混，不会
            bio_ids+=padding
            assert len(bio_ids) == len(pos_ids)
            ...
            if ex_id < 2:
                ...
                logger.info(f"bio_ids: {' '.join([str(x) for x in bio_ids])}")
    ...
    def create_dataset(self, features, is_sorted=False):
        ...
        if features[0].know_ids :
            ...
            all_bio_ids = torch.tensor([f.bio_ids for f in features], dtype=torch.long)
            ...
            all_bio_ids = torch.tensor([0 for f in features], dtype=torch.long)
        ...
        dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_boundary_ids, all_pos_ids,
                                all_rel_ids, all_know_ids,all_bio_ids)
        return dataset
