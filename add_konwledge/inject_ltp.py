...
'处所', '信息', '自然物', '鸟', '量化属性', '性格', '相对时间', '器具', '电器', '药物', '自然现象', '时段', '身份', '模糊属性', '职业', '爬行动物',
 ...
 '职业','组织机构', '组织','机构', '人物', '地名','地点' '城市', '数量', '疾病', '器械', '称谓', '时间', '职务', '工具', '手术', '车辆', '药物', '用具', '数目', '地点','公司',
'口语']
...
def conll(fr):
    ...
    for sent in tqdm(sents):
        ...
        ori_sent=''.join(words)
        # split_sent = tokenizer.segment(ori_sent)
        ...
        for index, pos in enumerate(postags):
            ...
            for seidx, (s,e) in enumerate(entity_s_e):
                if index>0 and s==boundary_ori[index-1]-1 and e==boundary_ori[index]:
                    for kw in key_word:
