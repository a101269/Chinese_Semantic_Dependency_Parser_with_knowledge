...
class CRF(nn.Module):
    ...
    def _viterbi_decode(self, feats, mask=None):
        ...
        if self.use_cuda:
            pad_zero=torch.cuda.LongTensor(batch_size, tag_size)
            pad_zero *= 0
        else:
        ...
        back_points = back_points.transpose(1, 0).contiguous()  # back_points.shape)torch.Size([76, 12, 44])
        ...
        if self.use_cuda:
            decode_idx = torch.cuda.LongTensor(seq_len, batch_size)
        else:
            decode_idx = torch.LongTensor(seq_len, batch_size)
    ...
    def _score_sentence(self, scores, mask, tags):
        ...
        if self.use_cuda:
            new_tags = torch.cuda.LongTensor(batch_size, seq_len)
        else :
